...
describe('teaser, content', function() {
    ...
    const checkAnchorCleanups = (html, $) => {
        ...
        // A fragment carrying a relative path (as emitted by the
        // document-viewers plugin, e.g.
        // /vendor/Viewer.js/index.html#../../../doc.pdf) is opaque to
        // path arithmetic: relativization must rewrite only the path
        // portion and re-append the fragment verbatim.
        assert.equal($('article#original a#fragment-preserved').attr('href'),
            'hier/index.html#../../../img/test.pdf');
        assert.equal($('article#original a#fragment-preserved').html(), 'Fragment preserved');

        // A fragment-only href (a same-page anchor, as generated by the
        // footnotes plugin's ak_footnoteRef partial) must pass through
        // untouched rather than tripping local-link resolution.
        assert.equal($('article#original a#same-page-anchor').attr('href'), '#section-name');
        assert.equal($('article#original a#same-page-anchor').html(), 'Same page anchor');
