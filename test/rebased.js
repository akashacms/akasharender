...
describe('rebased stylesheets, javascripts', function() {
    it('should find stylesheets, javascript values', async function() {
        ...
        assert.equal($('head link[rel="stylesheet"][type="text/css"][href="/rebase/to/vendor/bootstrap/css/bootstrap.min.css"]').length, 1);
        assert.equal($('head link[rel="stylesheet"][type="text/css"][href="/rebase/to/style.css"]').length, 1);
        assert.equal($('head link[rel="stylesheet"][type="text/css"][media="print"][href="/rebase/to/print.css"]').length, 1);

        assert.equal($('head script[src="/rebase/to/vendor/header-js.js"]').length, 1);
        assert.equal($('head script[type="no-known-lang"][src="/rebase/to/vendor/popper.js/popper.min.js"]').length, 1);
        assert.equal($('head script:contains("alert(\'in header with inline script\');")').length, 1);

        assert.equal($('body script[src="/rebase/to/vendor/jquery/jquery.min.js"]').length, 1);
        assert.equal($('body script[type="no-known-lang"][src="/rebase/to/vendor/popper.js/umd/popper.min.js"]').length, 1);
        assert.equal($('body script[src="/rebase/to/vendor/bootstrap/js/bootstrap.min.js"]').length, 1);
    });
    ...
    it('should find stylesheets, javascript from metadata values', async function() {
        ...
        assert.equal($('head link[rel="stylesheet"][type="text/css"][href="/rebase/to/vendor/metadata-css-added.css"]').length, 1);
        assert.equal($('head link[rel="stylesheet"][type="text/css"][media="print"][href="/rebase/to/vendor/metadata-css-with-media-added.css"]').length, 1);

        assert.equal($('head script[src="/rebase/to/vender/metadata-js-add-top.js"]').length, 1);
        assert.equal($('head script[type="no-known-lang"][src="/rebase/to/vender/metadata-js-with-lang-add-top.js"]').length, 1);
        assert.equal($('head script:contains("alert(\'added to top from metadata\');")').length, 1);


        assert.equal($('body script[src="/rebase/to/vender/metadata-js-add-bottom.js"]').length, 1);
        assert.equal($('body script[type="no-known-lang"][src="/rebase/to/vender/metadata-js-with-lang-add-bottom.js"]').length, 1);
...
describe('rebased header metadata', function() {
    ...
    it('should find RSS header meta', async function() {
        let { html, $ } = await akasha.readRenderedFile(config_rebase, 'index.html');

        assert.exists(html, 'result exists');
        assert.isString(html, 'result isString');

        assert.equal($('head link[rel="alternate"][type="application/rss+xml"][href="/rebase/to/rss-for-header.xml"]').length, 1);
    });
    ...
    it('should find xml-sitemap values', async function() {
        ...
        assert.equal($('head link[rel="sitemap"][type="application/xml"][title="Sitemap"][href="/rebase/to/sitemap.xml"]').length, 1);
        assert.equal($('head link[rel="sitemap"][type="application/xml"][title="Foo Bar Sitemap"][href="/rebase/to/foo-bar-sitemap.xml"]').length, 1);
    });
});
...
describe('rebased teaser, content', function() {
    it('should render fig-img', async function() {
        ...
        assert.equal($('article#original figure img[src="/rebase/to/simple-fig-img.jpg"]').length, 1);
        assert.equal($('article#original figure#with-caption img[src="/rebase/to/fig-img-caption.jpg"]').length, 1);
        assert.include($('article#original figure#with-caption figcaption').html(), 'Caption text');
        assert.equal($('article#original figure.added-class#with-class img[src="/rebase/to/fig-img-class.jpg"]').length, 1);
        assert.equal($('article#original figure#width[width="200px;"] img[src="/rebase/to/fig-img-width.jpg"]').length, 1);
        assert.equal($('article#original figure#style[style="border: 200 solid black;"] img[src="/rebase/to/fig-img-style.jpg"]').length, 1);
        assert.equal($('article#original figure#dest a[href="http://dest.url"]').length, 1);
        assert.equal($('article#original figure#dest img[src="/rebase/to/fig-img-dest.jpg"]').length, 1);
    });
    ...
    it('should render show-content', async function() {
        ...
        assert.equal($('article#original span#simple a[href="/rebase/to/shown-content.html"]').length, 1);
        assert.include($('article#original span#simple a[href="/rebase/to/shown-content.html"]').html(), 
        ...
        assert.equal($('article#original div#template figure a[href="/rebase/to/shown-content.html"] img[src="/rebase/to/imgz/shown-content-image.jpg"]').length, 1);
        assert.equal($('article#original div#template figure figcaption').length, 1);
        assert.equal($('article#original div#template figure figcaption a[href="/rebase/to/shown-content.html"]').length, 1);
        assert.include($('article#original div#template figure figcaption a[href="/rebase/to/shown-content.html"]').html(), 
            "Shown Content - solely for use of show-content.html");
        
        assert.equal($('article#original div#template2').length, 1);
        assert.equal($('article#original div#template2 figure').length, 1);
        assert.equal($('article#original div#template2 figure a[href="http://dest.url"] img[src="/rebase/to/imgz/shown-content-image.jpg"]').length, 1);
    ...
    it('should process anchor cleanups', async function() {
        ...
        assert.equal($('article#original a#convert-to-absolute-path').attr('href'), '/rebase/to/shown-content.html');
        ...
        assert.equal($('article#original a#insert-title-from-document').attr('href'), '/rebase/to/shown-content.html');
        ...
        assert.equal($('article#original a#img-causes-no-modify').attr('href'), '/rebase/to/shown-content.html');
