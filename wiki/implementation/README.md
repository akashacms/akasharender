...
- **[Removing the Legacy Non-2 Render Functions and Renaming the 2 Functions](./removing-legacy-render-functions.md)**: House-cleaning plan to delete the legacy `render`/`renderDocument`/`renderPath` (and their orphaned helpers `renderContent`, `writeCSStoOutput`, `copyAssetToOutput`, `data.report`, `data.data4file`), transition the three test files and the akasharender-epub plugin to the "2" API, rename the "2" functions to the base names in one breaking release, and purge stale `lib/watchman.ts` references from documentation.
