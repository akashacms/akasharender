---
title: Architecture index
---

# Architecture Pages

This directory contains high-level architecture documentation for major subsystems in AkashaRender.

## Architecture Documents

- **[Sitemap Validation Architecture](./sitemap-validation.md)**: Architecture design for validating generated sitemaps against local filesystem. Covers current sitemap generation process, research on existing tools, and proposed validator design with URL-to-filesystem mapping algorithm.
- **[Migrating AkashaRender to promised.node.sqlite](./promised-node-sqlite-migration.md)**: Architecture and phased migration plan for replacing the `sqlite3` + `promised-sqlite3` stack with the standalone `promised.node.sqlite` package built on `node:sqlite`. Covers the affected files, API compatibility, behavioral differences (extension loading, profiling, key-value store, undefined/boolean binding), the target design, and risks.
- **[oEmbed Provider Architecture for AkashaCMS](./oembed-provider.md)**: Design for making AkashaCMS an oEmbed provider via precomputed JSON/XML files referenced from `<head>` `<link>` tags. Confirms the issue #36 reasoning for `<link>`-based discovery, places the feature in `@akashacms/plugins-base`, and reuses the plugin's existing head-injection and `onSiteRendered` patterns behind a config flag.
- **[Breadcrumb Trail Schema.org Markup Architecture](./breadcrumb-schema-markup.md)**: Why `@akashacms/plugins-breadcrumbs` emits schema.org `BreadcrumbList` as inline microdata, documents the JSON-LD alternative, and records that JSON-LD is intentionally not implemented because it requires absolute URLs while AkashaCMS renders relative URLs.
- **[Vendor Library Plugins Architecture](./vendor-library-plugins.md)**: The repeatable pattern for wrapping 3rd-party JS/CSS/SVG libraries as AkashaCMS plugins — mount the package's assets on the stacked asset directories at `/vendor/LIBRARY-NAME`, then register CSS/JS tags. Explains the `addAssetsDir` / `addStylesheet` / `addFooterJavaScript` APIs, `import.meta.resolve` package location, the four existing plugins (bootstrap-icons, country-flag-icons, tabler-icons, theme-bootstrap), the optional Mahabhuta custom-tag layer, and a step-by-step recipe for new libraries.

## Suggested Topics

Based on the ingested code, the following architecture documents would be valuable:

- **Caching System Architecture**: How the four caches (documents, assets, layouts, partials) work together
- **Rendering Pipeline Architecture**: Flow from source files to final output
- **Plugin Architecture**: How plugins extend the system and interact with the core
- **Tag System Architecture**: How tags are stored, queried, and managed
